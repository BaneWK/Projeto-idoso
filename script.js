(function(){
  ...
  document.getElementById('loginForm')?.addEventListener('submit', async (e) => {
    ...
    try {
      // Modificado para ignorar os inputs e enviar credenciais demonstrativas automáticas ao back-end
      const dados = await apiFetch('/auth/login', {
        method: 'POST',
        body: JSON.stringify({ email: 'admin@laco.com', password: '123456' })
      });

      localStorage.setItem('laco_token', dados.token);
      localStorage.setItem('laco_usuario', dados.user.email);
      localStorage.setItem('laco_perfil', dados.user.role); // 'admin' ou 'idoso'
